...
if "person" in st.session_state:
    ...
    def generate_summary_advice(person, cbc_cols, blood_cols, sex):
        ...
        for label, col, low, high, tip, short in [
            ...
            ("Monocyte", "mo", 3, 9, "3 - 9%", "อักเสบ"),
            ("Eosinophil", "eo", 0, 9, "0 - 9%", "ภูมิแพ้"),
            ("Basophil", "ba", 0, 3, "0 - 3%", "การอักเสบ"),
        ]:
            val = person.get(cbc_cols.get(col))
            if abnormal(val, low, high) != "normal":
                add(f'{highlight(f"{label} ผิดปกติ", val, f"ค่าปกติ {tip}")} - อาจเกี่ยวกับ{short}')
                ...
                for label, col, low, high, tip, rec, hib, critical in [
                    ...
                    result = abnormal(val, low, high, higher_is_better=hib)
                    if result != "normal":
                        if critical:
                            add(f"{label} ผิดปกติ ({val}) - ค่าดังกล่าวอยู่นอกช่วงมาตรฐาน ({tip}) ควรพบแพทย์เพื่อตรวจเพิ่มเติม", crit=True)
                        else:
                            add(f"{label} ผิดปกติ ({val}) - ค่าดังกล่าวอยู่นอกช่วงปกติ ({tip}) แนะนำ {rec}")
    
        def classify_status(val, low=None, high=None):
            status = abnormal(val, low, high)
            return {
                "low": "ต่ำกว่าเกณฑ์",
                "high": "สูงกว่าเกณฑ์",
                "normal": "ปกติ",
        ...
        def cbc_advice(hb_status, wbc_status, plt_status):
            if all(x in ["", "-", None] for x in [hb_status, wbc_status, plt_status]):
                return ""
    
            hb = hb_status.strip()
            wbc = wbc_status.strip()
            plt = plt_status.strip()
    
            if plt in ["ต่ำกว่าเกณฑ์", "ต่ำกว่าเกณฑ์เล็กน้อย"]:
                return "ค่าเกล็ดเลือดต่ำผิดปกติ ควรพบแพทย์เพื่อตรวจหาสาเหตุและเฝ้าระวังอาการผิดปกติ"
    
            if hb == "ปกติ" and wbc == "ปกติ" and plt == "ปกติ":
                return ""
    
            if hb == "พบภาวะโลหิตจาง" and wbc == "ปกติ" and plt == "ปกติ":
                return "พบค่าภาวะโลหิตจาง ควรพบแพทย์เพื่อตรวจเพิ่มเติมและติดตามรักษา"
    
            if hb == "พบภาวะโลหิตจาง" and wbc in ["ต่ำกว่าเกณฑ์", "ต่ำกว่าเกณฑ์เล็กน้อย", "สูงกว่าเกณฑ์เล็กน้อย", "สูงกว่าเกณฑ์"]:
                return "พบค่าผิดปกติของเม็ดเลือดแดงและขาว ควรพบแพทย์เพื่อตรวจเพิ่มเติมและพิจารณาการรักษา"
    
            if hb == "พบภาวะโลหิตจางเล็กน้อย" and wbc == "ปกติ" and plt == "ปกติ":
                return "ภาวะโลหิตจางเล็กน้อย แนะนำดูแลสุขภาพและรับประทานอาหารที่มีธาตุเหล็ก"
    
            if hb == "ปกติ" and wbc in ["ต่ำกว่าเกณฑ์", "ต่ำกว่าเกณฑ์เล็กน้อย", "สูงกว่าเกณฑ์เล็กน้อย", "สูงกว่าเกณฑ์"]:
                return "เม็ดเลือดขาวอยู่นอกเกณฑ์เล็กน้อย ควรดูแลสุขภาพและติดตามค่าซ้ำ"
    
            if plt == "สูงกว่าเกณฑ์":
                return "ค่าเกล็ดเลือดสูง ควรพบแพทย์เพื่อตรวจเพิ่มเติมและพิจารณาสาเหตุ"
    
            if hb == "พบภาวะโลหิตจางเล็กน้อย" and \
               wbc in ["ต่ำกว่าเกณฑ์", "ต่ำกว่าเกณฑ์เล็กน้อย", "สูงกว่าเกณฑ์เล็กน้อย", "สูงกว่าเกณฑ์"] and plt == "ปกติ":
                return "พบค่าผิดปกติของเม็ดเลือดแดงและขาว ควรดูแลสุขภาพและพิจารณาการติดตามค่าซ้ำ"
    
            return "ค่าผิดปกติในระบบเม็ดเลือด ควรพบแพทย์เพื่อตรวจเพิ่มเติม"
    
        # ✅ ตรวจสถานะ CBC และเพิ่มข้อความสรุป
        hb_status = "พบภาวะโลหิตจาง" if abnormal(hb, low=hb_limit) == "low" else "ปกติ"
        wbc_status = classify_status(wbc, 4000, 10000)
        plt_status = classify_status(plt, 150000, 500000)
    
        cbc_summary = cbc_advice(hb_status, wbc_status, plt_status)
        if cbc_summary:
            add(cbc_summary)
