...
if "person" in st.session_state:
    ...
    # ======================= SUMMARY ADVICE GROUPED BY SYSTEM =======================

    def abnormal(value, low=None, high=None, higher_is_better=False):
        try:
    ...
    # 🩸 CBC
    if abnormal(person.get(cbc_cols.get("hb")), low=hb_low):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("ค่า Hb ต่ำ อาจมีภาวะโลหิตจาง")
    if abnormal(person.get(cbc_cols.get("hct")), low=hct_low):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("ค่า Hct ต่ำ อาจสัมพันธ์กับการสูญเสียเลือด")
    if abnormal(person.get(cbc_cols.get("wbc")), 4000, 10000):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("เม็ดเลือดขาวผิดปกติ อาจเกิดการติดเชื้อหรือภาวะภูมิคุ้มกัน")
    if abnormal(person.get(cbc_cols.get("plt")), 150000, 500000):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("เกล็ดเลือดผิดปกติ อาจมีผลต่อระบบการแข็งตัวของเลือด")
    # เพิ่มแนะนำค่าผิดปกติของเม็ดเลือดแยกย่อย
    if abnormal(person.get(cbc_cols.get("ne")), 43, 70):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("นิวโทรฟิล (Neutrophil) ผิดปกติ อาจสัมพันธ์กับการติดเชื้อหรือภูมิคุ้มกัน")
    
    if abnormal(person.get(cbc_cols.get("ly")), 20, 44):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("ลิมโฟไซต์ (Lymphocyte) ผิดปกติ อาจเกิดจากการติดเชื้อไวรัสหรือปัญหาภูมิคุ้มกัน")
    
    if abnormal(person.get(cbc_cols.get("mo")), 3, 9):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("โมโนไซต์ (Monocyte) ผิดปกติ อาจสัมพันธ์กับการอักเสบหรือการติดเชื้อเรื้อรัง")
    
    if abnormal(person.get(cbc_cols.get("eo")), 0, 9):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("อีโอซิโนฟิล (Eosinophil) ผิดปกติ อาจสัมพันธ์กับการแพ้หรือปรสิต")
    
    if abnormal(person.get(cbc_cols.get("ba")), 0, 3):
        grouped_advice["🩸 เม็ดเลือด (CBC)"].append("เบโซฟิล (Basophil) ผิดปกติ อาจเกี่ยวข้องกับการอักเสบหรือโรคภูมิแพ้")

    
    # 🍬 น้ำตาล
    if abnormal(person.get(blood_cols["FBS"]), 74, 106):
        grouped_advice["🍬 น้ำตาลในเลือด"].append("ระดับน้ำตาลในเลือดผิดปกติ อาจเสี่ยงเบาหวาน")
    
    # 🍔 ไขมัน
    if abnormal(person.get(blood_cols["Cholesterol"]), 150, 200):
        grouped_advice["🍔 ไขมันในเลือด"].append("คอเลสเตอรอลสูง อาจเพิ่มความเสี่ยงโรคหัวใจ")
    if abnormal(person.get(blood_cols["TG"]), 35, 150):
        grouped_advice["🍔 ไขมันในเลือด"].append("ไตรกลีเซอไรด์สูง อาจเสี่ยงโรคหลอดเลือด")
    if abnormal(person.get(blood_cols["HDL"]), low=40, higher_is_better=True):
        grouped_advice["🍔 ไขมันในเลือด"].append("ไขมันดี (HDL) ต่ำ ควรออกกำลังกายสม่ำเสมอ")
    if abnormal(person.get(blood_cols["LDL"]), 0, 160):
        grouped_advice["🍔 ไขมันในเลือด"].append("ไขมันเลว (LDL) สูง อาจเพิ่มความเสี่ยงหลอดเลือดแข็ง")
    
    # 🧪 ตับ
    if abnormal(person.get(blood_cols["ALK"]), 30, 120):
        grouped_advice["🧪 การทำงานของตับ"].append("เอนไซม์ ALK สูง อาจเกิดภาวะตับอักเสบ")
    if abnormal(person.get(blood_cols["SGOT"]), None, 37):
        grouped_advice["🧪 การทำงานของตับ"].append("SGOT สูง อาจมีการอักเสบของตับหรือกล้ามเนื้อ")
    if abnormal(person.get(blood_cols["SGPT"]), None, 41):
        grouped_advice["🧪 การทำงานของตับ"].append("SGPT สูง อาจเกี่ยวข้องกับตับอักเสบ")
    
    # 💧 ไต
    if abnormal(person.get(blood_cols["BUN"]), 7.9, 20):
        grouped_advice["💧 การทำงานของไต"].append("ค่า BUN ผิดปกติ อาจต้องติดตามการทำงานของไต")
    if abnormal(person.get(blood_cols["Cr"]), 0.5, 1.17):
        grouped_advice["💧 การทำงานของไต"].append("ค่า Creatinine สูง อาจบ่งชี้ภาวะไตเสื่อม")
    if abnormal(person.get(blood_cols["GFR"]), low=60, higher_is_better=True):
        grouped_advice["💧 การทำงานของไต"].append("ค่า GFR ต่ำ อาจเป็นสัญญาณของภาวะไตเรื้อรัง")
    
    # รวมเป็น HTML แสดงผล
    summary_html = []
    for group, items in grouped_advice.items():
        if items:
            html_block = f"<b>{group}</b><ul>" + "".join(f"<li>{msg}</li>" for msg in items) + "</ul>"
            summary_html.append(html_block)
    ...
    if summary_html:
        ...
        <div style='background-color:#2e0e0e33; padding: 20px; border-left: 6px solid #ff4d4d; border-radius: 8px; margin-top: 24px;'>
            ...
            {"".join(summary_html)}
            <p style='margin-top:12px;'>📞 แนะนำให้ปรึกษาแพทย์หากมีอาการผิดปกติ หรือเพื่อประเมินเพิ่มเติม</p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
