    ...
    def generate_summary_advice(person, cbc_cols, blood_cols, sex):
        ...
        for label, col, low, high, tip, rec, hib, critical in [
            ...
            if abnormal(val, low, high, higher_is_better=hib):
                if critical:
                    messages.append(
                        f"🔸 {label} ผิดปกติ ({val}) - ค่าดังกล่าวอยู่นอกช่วงมาตรฐาน ({tip}) "
                        f"ควรพบแพทย์เพื่อเข้ารับการตรวจเพิ่มเติม"
                    )
                else:
                    messages.append(
                        f"🔸 {label} ผิดปกติ ({val}) - ค่าดังกล่าวอยู่นอกช่วงปกติ ({tip}) "
                        f"แนะนำ {rec} และติดตามพฤติกรรมสุขภาพ"
                    )
    
        if not messages:
            return "✅ สุขภาพอยู่ในเกณฑ์ดี ไม่มีค่าผิดปกติ"
    
        return "<br>".join(messages) + "<br><br><b>📌 หากพบ <span style='color:red;'>🚩</span> ควรพบแพทย์เพื่อติดตามเพิ่มเติม</b>"
